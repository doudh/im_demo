package com.ddh.message;

import java.util.Date;
import java.util.UUID;

import com.ddh.bean.ClientInfo;
import com.ddh.repository.ClientInfoRepository;
...
public class MessageEventHandler {
    ...
    public void onConnect(SocketIOClient client) {
        ...
        if ("testclient2".equals(clientId)||"testclient3".equals(clientId)) {
    ...
    public void onEvent(SocketIOClient client, AckRequest ackRequest, MessageInfo data){
        ...
        if (clientInfo != null && clientInfo.getConnected() != 0){
            ...
            server.getClient(uuid).sendEvent("messageevent", sendData,"read");
        }
    }

    //消息接收入口，当接收到消息后，查找发送目标客户端，并且向该客户端发送消息，且给自己发送消息
    @OnEvent(value = "chat")
    public void chat(SocketIOClient client, AckRequest ackRequest, MessageInfo data){

        server.getRoomOperations("room").sendEvent("room", data);
    }
